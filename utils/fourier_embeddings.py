...
from utils.preprocessing import resize_image, measure_snr, measure_noise
from utils.common import multiprocess, gaussian_kernel, fft, ifft, normalize_otf
...
def fourier_embeddings(
    ...
    if np.all(psf == 0):
        if digital_rotations is not None:
            emb = np.zeros((digital_rotations, 6, *model_psf_shape[1:]))
        else:
            emb = np.zeros((6, *model_psf_shape[1:]))
...
def rolling_fourier_embeddings(
    ...
    if np.all(rois == 0):
        ...
        if emb.shape[1:] != model_psf_shape[1:]:
            emb = resize_image(emb, crop_shape=(6, *model_psf_shape[1:]), interpolate=True)
